...
export default function EventCategoriesPage() {
  ...
  function load() {
    void apiGet<CategoryRow[]>('/admin/events/categories').then((result) => {
      setRows(result.data || []);
    });
  }
  ...
  async function create() {
    const result = await apiPost('/admin/events/categories', { name });
    if (!result.success) {
    ...
    <PermissionGate permission="events.view">
      <div className="space-y-6">
        <PageHeader
          ...
          description="Configurable event types. Example names are only suggestions until staff create them."
        />
        {can('events.create') ? (
          <form
        ...
        <ConfirmDialog
          ...
          onConfirm={async () => {
            if (!pendingId) return;
            const result = await apiDelete(`/admin/events/categories/${pendingId}`);
            if (!result.success) {
              toast.error(publicErrorMessage(result.status, result.message));
              return;
            }
