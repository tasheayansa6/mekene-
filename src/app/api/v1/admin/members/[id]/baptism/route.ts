...
const schema = z.object({
  ...
  baptismCategory: z.string().trim().max(80).nullable().optional(),
...
export async function GET(request: Request, context: RouteContext) {
  ...
  const rows = await db.baptismRecord.findMany({
    where: { memberId: id },
  ...
  return success(
    rows.map((row) => ({
      id: row.id,
      baptismDate: row.baptismDate.toISOString(),
      location: row.location,
      officiant: row.officiant,
      baptismCategory: row.baptismCategory,
...
export async function POST(request: Request, context: RouteContext) {
  ...
  const baptismDate = new Date(parsed.data.baptismDate);
  if (Number.isNaN(baptismDate.getTime())) {
    return validationError({ baptismDate: ['Invalid date.'] });
  }
  const row = await db.baptismRecord.create({
    data: {
      memberId: id,
      baptismDate,
      ...
      baptismCategory: parsed.data.baptismCategory
        ? sanitizePlainText(parsed.data.baptismCategory, 80)
        : null,
  ...
  await db.member.update({
    where: { id },
    data: {
      baptismDate: member.baptismDate || baptismDate,
      baptismNote: member.baptismNote || row.notes,
    },
  });
  return success(row, 'Baptism record created.', 201);
}
