...
import { cancelLiveSession, getLiveSessionById } from '@/lib/live/sessions';
...
export async function POST(request: Request, context: RouteContext) {
  ...
  const limited = enforceAdminRateLimit(request, auth.user.id, 'live-cancel');
  ...
  const result = await cancelLiveSession(id, auth.user.id, request);
  if (!result.ok) return error(result.error, 400);
  return success(serializeAdminLiveSession(result.session), 'Live session cancelled.');
}
